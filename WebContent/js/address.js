...
$(function() {
  ...
  $.each(cityJson, function(i, val) {
      if (val.item_code.substr(2, 4) == '0000') {
    	  sb.append("<option value='" + val.item_name + "'>" + val.item_name + "</option>");
      }
    });
  $("#choosePro").after(sb.toString());  // 在选定元素后面插入指定内容
}); 
...
function doProvAndCityRelation() {
  ...
  if ($("#chooseCity").length === 0) {
    city.append("<option id='chooseCity'>城市</option>");
  }
  if ($("#chooseCounty").length === 0) {
    county.append("<option id='chooseCounty'>区/县</option>");
  }
  var sb = new StringBuffer();
  $.each(cityJson, function(i, val){
	  if(val.item_name == $("#province").val()){
		  $id = val.item_code;
	  }
	  if (val.item_code.substr(0, 2) == $id.substr(0, 2) && val.item_code.substr(2, 4) != '0000' && val.item_code.substr(4, 2) == '00') {
		  sb.append("<option value='" + val.item_name + "'>" + val.item_name + "</option>");
	  }
  })
...
//  var cityVal = $("#citys").val();
  ...
  if ($("#chooseCounty").length === 0) {
    county.append("<option id='chooseCounty'>县/区</option>");
  }
  var sb = new StringBuffer();
  $.each(cityJson, function(i, val) {
	  if (val.item_name == $("#citys").val()) {
		  $id = val.item_code;
	  }
      if ($id == '110100' || $id == "120100" || $id == "310100" || $id == "500100") {
        if (val.item_code.substr(0, 3) == $id.substr(0, 3) && val.item_code.substr(4, 2) != '00') {
          sb.append("<option value='" + val.item_name + "'>" + val.item_name + "</option>");
        }
      } else {
        if (val.item_code.substr(0, 4) == $id.substr(0, 4) && val.item_code.substr(4, 2) != '00') {
          sb.append("<option value='" + val.item_name + "'>" + val.item_name + "</option>");
        }
      }
    });
