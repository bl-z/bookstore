...
function addPrint() {
	$('#printInfo').dialog('open').dialog('setTitle', '请正确的输入印刷厂的相关信息');
	$('#pritnFm').form('clear');
...
function editPrint() {
	...
	var row = $('#printTB').datagrid('getSelected');
	if (row) {
		$('#printInfo').dialog('open').dialog('setTitle',
				'修改印刷厂的信息！注意：印刷商的编号不能修改...');
		$('#printFm').form('load', row);
...
function savePrint() {
	...
	$('#printFm').form('submit', {
		...
		onSubmit : function() {
			// 获取输入的数据    name  value
			var temp = $('#printFm').serializeArray();
			...
			return $('#printFm').form('validate');// jquery $("#")  $('#fm').form('validate') 会去调用验证框架进行验证，然后对所有easyui-validateBox验证结果进行与操作，全部ok表单才可以提交
		},
		success : function(result) {
			var result = eval('(' + result + ')');// 数据解析，如果是json数据就解析成json
			if (result.state) {
				if (url == 'addPrint'){
					$('#printTB').datagrid('appendRow', {
					...
					$('#printTB').datagrid('updateRow', {
						index : $_index,
						row : $form
					});
				}

				$('#printInfo').dialog('close'); // close the dialog
			}
			$.messager.show({
				title : '提示信息！',
				msg : result.msg
					...
					if (cfm) {
						...
						$.post('/bookstore/printController/removePrint', {
							...
							if (result.state) {
								...
								$('#printTB').datagrid('deleteRow', $_index)
							} else {
